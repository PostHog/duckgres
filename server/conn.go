...
// portalState describes whether a portal can still be executed. Duckgres does
// not yet implement PortalSuspended, so every Execute reaches a terminal state.
// Keeping the shell after completion matches PostgreSQL's error behavior for a
// repeated Execute while allowing the large Bind payload to be released now.
...
func (c *clientConn) messageLoop() error {
	...
	for {
		...
		case wire.MsgSync:
			...
			// recovery, then reports readiness. In idle state it also ends the
			// implicit transaction and drops portal shells; explicit T/E state
			// preserves portals as PostgreSQL clients expect.
			c.ignoreTillSync = false
			if c.txStatus == txStatusIdle {
				c.dropAllPortals("idle_sync")
			}
			if err := c.writeReadyForQuery(c.txStatus); err != nil {
				return err
			}
