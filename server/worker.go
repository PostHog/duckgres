...
func runChildWorker(tcpConn *net.TCPConn, cfg *ChildConfig) int {
	...
	// Create a worker server (minimal server instance for the child)
	worker := &workerServer{
		cfg:       serverCfg,
		tlsConfig: tlsConfig,
	}
	...
	db, err := worker.createDBConnection(username)
	if err != nil {
...
// workerServer is a minimal server implementation for child worker processes.
// It provides the database connection creation logic without the full server infrastructure.
