...
func TestIsNativeDuckDBCommand(t *testing.T) {
	c := &clientConn{}
	...
	tests := []struct {
		...
		{
			name:       "SET native_duckdb = true",
			query:      "SET native_duckdb = true",
			expectIsSet:  true,
			expectIsShow: false,
			expectValue:  true,
		},
		{
			name:       "SET native_duckdb = 1",
			query:      "SET native_duckdb = 1",
			expectIsSet:  true,
			expectIsShow: false,
			expectValue:  true,
		},
		...
		{
			name:       "SET native_duckdb TO on",
			query:      "SET native_duckdb TO on",
			expectIsSet:  true,
			expectIsShow: false,
			expectValue:  true,
		},

		// SET commands - off/false values
		{
		...
		{
			name:       "SET native_duckdb = false",
			query:      "SET native_duckdb = false",
			expectIsSet:  true,
			expectIsShow: false,
			expectValue:  false,
		},
		{
			name:       "SET native_duckdb = 0",
			query:      "SET native_duckdb = 0",
			expectIsSet:  true,
			expectIsShow: false,
			expectValue:  false,
		},
		...
		},
		{
			...
			query:      "SET native_duckdb = invalid",
			expectIsSet:  false,
			expectIsShow: false,
			expectValue:  false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			isSet, isShow, value := c.isNativeDuckDBCommand(tt.query)
			if isSet != tt.expectIsSet {
				t.Errorf("isSet = %v, want %v", isSet, tt.expectIsSet)
			}
			if isShow != tt.expectIsShow {
				t.Errorf("isShow = %v, want %v", isShow, tt.expectIsShow)
			}
			if isSet && value != tt.expectValue {
				t.Errorf("value = %v, want %v", value, tt.expectValue)
			}
		})
	}
}

func TestNativeDuckDBModeToggle(t *testing.T) {
	// Test that nativeDuckDB field can be toggled
	c := &clientConn{nativeDuckDB: false}

	// Initially off
	if c.nativeDuckDB {
		t.Errorf("expected nativeDuckDB to be false initially")
	}

	// Toggle on
	c.nativeDuckDB = true
	if !c.nativeDuckDB {
		t.Errorf("expected nativeDuckDB to be true after toggle")
	}

	// Toggle off
	c.nativeDuckDB = false
	if c.nativeDuckDB {
		t.Errorf("expected nativeDuckDB to be false after second toggle")
	}
}
