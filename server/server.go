...
//
// This MUST run after the catalog ATTACH calls, not in ConfigureMainDB. httpfs
// is auto-loaded by DuckLake's ATTACH on the first S3 touch, so it is NOT in
...
func ConfigureDBConnection(db *sql.DB, cfg Config, duckLakeSem chan struct{}, username string, serverStartTime time.Time, serverVersion string) error {
	...
	// httpfs. See applyHTTPFSRetryBudget for why this can't run in ConfigureMainDB.
...
func ActivateDBConnection(db *sql.DB, cfg Config, duckLakeSem chan struct{}, username string) error {
	...
	// sessionmeta.InitSessionDatabaseMetadata for each Postgres session.
