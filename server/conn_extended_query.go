...
func (c *clientConn) handleBind(body []byte) {
	...
	// Validate result-format vectors when their cardinality is already known.
	// Unknown mutation output is deferred until actual metadata is available.
...
func bindResultFormatCountSafeBeforeExecute(ps *preparedStmt, formats int) bool {
	...
	if len(ps.statements) > 0 {
		if !queryReturnsResults(ps.statements[len(ps.statements)-1]) {
			return false
		}
		// TODO: validate deferred mutation result formats before execution
		// without running the mutation merely to discover its schema.
		return true
	}
	if isDMLReturning(ps.query) {
