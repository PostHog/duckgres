...
func TestSessionStartScopeRecordsExactlyOnce(t *testing.T) {
	...
	terminalBefore := postgresSessionStartCount(t, org, "success", SessionStartFailureNone)
	failureBefore := postgresSessionStartCount(t, org, "failure", SessionStartFailureControlPlane)

	scope := BeginSessionStart(org, "postgres")
	scope.Finish("success", SessionStartFailureControlPlane)
	scope.Finish("error", SessionStartFailureControlPlane)
	...
	if got := postgresSessionStartCount(t, org, "success", SessionStartFailureNone); got != terminalBefore+1 {
		t.Fatalf("terminal success count = %v, want %v", got, terminalBefore+1)
	}
	if got := postgresSessionStartCount(t, org, "failure", SessionStartFailureControlPlane); got != failureBefore {
...
func TestSessionStartScopeRecordsPostgresTerminalFailure(t *testing.T) {
	...
	before := postgresSessionStartCount(t, org, "failure", SessionStartFailureMetadataStore)
	histogramBefore := sessionStartSampleCount(t, org, "postgres", "timeout")

	BeginSessionStart(org, "postgres").Finish("timeout", SessionStartFailureMetadataStore)

	if got := postgresSessionStartCount(t, org, "failure", SessionStartFailureMetadataStore); got != before+1 {
...
func TestSessionStartScopeNormalizesUnsafeFailureClasses(t *testing.T) {
	tests := []struct {
		name         string
		outcome      string
		failureClass string
	...
	}{
		{
			...
			outcome:      "success",
			failureClass: SessionStartFailureWorker,
			wantOutcome:  "success",
			wantClass:    SessionStartFailureNone,
		},
		{
			name:         "failure cannot use none",
			outcome:      "error",
			failureClass: SessionStartFailureNone,
			wantOutcome:  "failure",
			wantClass:    SessionStartFailureUnknown,
		},
		{
			name:         "unknown class stays non-pageable",
			outcome:      "error",
			failureClass: "new-unclassified-path",
			wantOutcome:  "failure",
			wantClass:    SessionStartFailureUnknown,
		},
	}
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			before := postgresSessionStartCount(t, org, tt.wantOutcome, tt.wantClass)

			BeginSessionStart(org, "postgres").Finish(tt.outcome, tt.failureClass)

			if got := postgresSessionStartCount(t, org, tt.wantOutcome, tt.wantClass); got != before+1 {
...
func TestSessionStartScopeDoesNotExportFlightTerminalCounter(t *testing.T) {
	...
	BeginSessionStart(org, "flight").Finish("error", SessionStartFailureMetadataStore)

	if got := postgresSessionStartCount(t, org, "failure", SessionStartFailureMetadataStore); got != before {
		t.Fatalf("obsolete Flight path changed PostgreSQL terminal count: got %v, want %v", got, before)
	}
}

func postgresSessionStartCount(t *testing.T, org, outcome, failureClass string) float64 {
	t.Helper()
	return counterVecValue(t, postgresSessionStartCounter, org, outcome, failureClass)
}
