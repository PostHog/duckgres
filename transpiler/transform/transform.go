...
type Result struct {
	...
	// SchemaQuery is a query that can be PREPAREd to discover the output schema
	// without executing user DML. Writable-CTE rewrites replace their writable
	// CTE bodies with the equivalent generated SELECTs; an outer DML statement
	// may remain because PREPARE itself is non-executing.
	SchemaQuery string
