...
func TestShouldRetryDuckLakeConflictOnlyAutocommit(t *testing.T) {
	err := errors.New("Transaction conflict: cannot commit")
	if !shouldRetryDuckLakeConflict(err, false) {
		t.Fatal("autocommit transaction conflict should retry")
	}
	if shouldRetryDuckLakeConflict(err, true) {
		t.Fatal("transaction conflict inside user transaction must not retry")
	}
	if shouldRetryDuckLakeConflict(errors.New("other error"), false) {
		t.Fatal("non-conflict error must not retry")
	}
}
...
func TestSQLTxActiveTrackingHandlesLeadingComments(t *testing.T) {
	var flag atomic.Bool

	trackSQLTransactionState("/*Fivetran*/ BEGIN", nil, &flag)
	if !flag.Load() {
		t.Fatal("sqlTxActive should be true after comment-prefixed BEGIN")
	}

	trackSQLTransactionState("/*Fivetran*/ COMMIT", nil, &flag)
	if flag.Load() {
		t.Fatal("sqlTxActive should be false after comment-prefixed COMMIT")
	}

	trackSQLTransactionState("-- generated by client\nSTART TRANSACTION", nil, &flag)
	if !flag.Load() {
		t.Fatal("sqlTxActive should be true after line-comment-prefixed START TRANSACTION")
	}

	trackSQLTransactionState("-- generated by client\nROLLBACK", nil, &flag)
	if flag.Load() {
		t.Fatal("sqlTxActive should be false after line-comment-prefixed ROLLBACK")
	}
}
