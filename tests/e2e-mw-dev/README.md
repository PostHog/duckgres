...
- **PostHog product-analytics events** (`internal/analytics`,
  `warehouse_provisioned`/`_deprovisioned`/`_password_reset`,
  `query_initiated`/`query_failed`) — the harness already drives every path
  that fires these (it provisions, deprovisions, resets, and runs queries on
  both metadata backends), but the events are sent asynchronously to PostHog's
  ...
  `controlplane/provisioning/analytics_events_test.go`, and
  `server/conn_analytics_test.go`.
