...
// key when present. This lets DNS-safe org-name hostnames route to DB-safe
// database names.
...
// TestSNI_MatchedHostnameFallsBackWhenDatabaseParamEmpty: when the client
// omits the startup database and the managed SNI prefix does not resolve to an
// org, the raw prefix remains the database-shaped error target.
func TestSNI_MatchedHostnameFallsBackWhenDatabaseParamEmpty(t *testing.T) {
	if err := waitForDBReady(60 * time.Second); err != nil {
		t.Fatalf("waitForDBReady: %v", err)
	}
	...
	_, err := connectWithSNI(ctx, "ghostorg"+sniManagedSuffix,
		"", sniSeedUser, sniSeedPassword)
	if err == nil {
		t.Fatalf("expected unknown managed SNI prefix to be rejected")
	}
