...
func TestDevScenarioWorkflowUsesUnifiedMwDevHarness(t *testing.T) {
	...
	for _, required := range []string{
		...
		"max-parallel: 1",
		"- name: Require a current image for cached comparisons",
		"Cached Trino comparisons require the control-plane image built from this revision.",
		`fromJSON('["posthog_frozen_perf", "posthog_frozen_perf_trino_cached"]')`,
		"PR_NUMBER: ${{ github.run_id }}${{ strategy.job-index }}",
		"NAMESPACE: duckgres-ci-pr-${{ github.run_id }}${{ strategy.job-index }}",
		"name: scenario-dev-${{ matrix.scenario }}-${{ github.run_id }}-${{ github.run_attempt }}",
		"id-token: write",
