...
# Dev-only Trino benchmark lifecycle (posthog_frozen_trino_perf). OFF unless a
# run supplies a pinned Trino+Brikk image; the flag alone does nothing. The
# harness passes ONLY the image and the flag — the metadata reader password and
# the read-only S3 role are charts-created Kubernetes resources the control
# plane resolves for itself, so no credential passes through this script.
...
if [ -n "$DUCKGRES_TRINO_BENCHMARK_IMAGE" ]; then
  DUCKGRES_TRINO_BENCHMARK_ENABLED=true
elif [ "$DUCKGRES_TRINO_BENCHMARK_ENABLED" = "true" ]; then
  echo "DUCKGRES_TRINO_BENCHMARK_ENABLED=true requires DUCKGRES_TRINO_BENCHMARK_IMAGE (a pinned Trino+Brikk image)." >&2
  exit 2
fi
...
render() {
  ...
  DUCKGRES_TRINO_BENCHMARK_ENABLED="$DUCKGRES_TRINO_BENCHMARK_ENABLED" \
  DUCKGRES_TRINO_BENCHMARK_IMAGE="$DUCKGRES_TRINO_BENCHMARK_IMAGE" \
    envsubst '$NAMESPACE $PR_NUMBER $WORKER_IMAGE $CONTROLPLANE_IMAGE $INTERNAL_SECRET $INTERNAL_SECRET_FALLBACK $USER_SECRET_KEY $DUCKGRES_K8S_WORKER_CPU_REQUEST $DUCKGRES_K8S_WORKER_MEMORY_REQUEST $DUCKGRES_TRINO_BENCHMARK_ENABLED $DUCKGRES_TRINO_BENCHMARK_IMAGE' \
