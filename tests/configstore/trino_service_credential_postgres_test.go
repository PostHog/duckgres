...
func TestTrinoServiceCredentialLifecyclePostgres(t *testing.T) {
	...
	for _, org := range []string{"acme", "other"} {
		if err := store.EnableTrino(org, configstore.TrinoSettings{Tier: "scale"}); err != nil {
			t.Fatal(err)
		}
	}
	...
	identity, err := store.ValidateTrinoServiceCredential(ctx, username, grant.Plaintext)
	...
	for _, user := range []string{"otherdb." + grant.CredentialID, "acmedb.svc_000000000000000000000000", "acmedb.root", grant.CredentialID} {
		if _, err := store.ValidateTrinoServiceCredential(ctx, user, grant.Plaintext); !errors.Is(err, configstore.ErrTrinoServiceCredentialDenied) {
	...
	if _, err := store.ValidateTrinoServiceCredential(ctx, username, grant.Plaintext); err != nil {
	...
	if _, err := store.ValidateTrinoServiceCredential(ctx, username, grant.Plaintext); !errors.Is(err, configstore.ErrTrinoServiceCredentialDenied) {
		t.Fatalf("old secret still authenticates: %v", err)
	}
	if _, err := store.ValidateTrinoServiceCredential(ctx, username, rotated.Plaintext); err != nil {
		t.Fatal(err)
	}
	if err := store.DisableTrino("acme"); err != nil {
		t.Fatal(err)
	}
	if _, err := store.ValidateTrinoServiceCredential(ctx, username, rotated.Plaintext); !errors.Is(err, configstore.ErrTrinoServiceCredentialDenied) {
		t.Fatalf("disabled org authenticates: %v", err)
	}
	if err := store.EnableTrino("acme", configstore.TrinoSettings{Tier: "growth"}); err != nil {
		t.Fatal(err)
	}
	identity, err = store.ValidateTrinoServiceCredential(ctx, username, rotated.Plaintext)
	...
	if _, err := store.ValidateTrinoServiceCredential(ctx, "acmedb."+grant.CredentialID, grant.Plaintext); !errors.Is(err, configstore.ErrTrinoServiceCredentialDenied) {
