...
func TestRunBootstrapOnlyMigratesWithoutPublishing(t *testing.T) {
	t.Parallel()

	var bootstrapped *publisher.Config
	deps := Dependencies{
		Stdin:  strings.NewReader(`{"host":"perf.example.test","port":5432,"database":"ducklake","username":"w","password":"p"}`),
		Stdout: io.Discard,
		Publish: func(context.Context, publisher.Config, string) error {
			t.Fatal("bootstrap-only must not publish a run")
			return nil
		},
		Bootstrap: func(_ context.Context, cfg publisher.Config) error {
			bootstrapped = &cfg
			return nil
		},
	}
	if err := Run(context.Background(), []string{"--bootstrap-only", "--connection-secret-stdin", "--schema", "duckgres_scenario_perf"}, deps); err != nil {
		t.Fatalf("Run returned error: %v", err)
	}
	if bootstrapped == nil || bootstrapped.Schema != "duckgres_scenario_perf" || bootstrapped.Password != "p" {
		t.Fatalf("bootstrap config = %#v", bootstrapped)
	}
}

func TestRunBootstrapOnlyRejectsRunDirAndDisabledBootstrap(t *testing.T) {
	t.Parallel()

	for name, args := range map[string][]string{
		"run dir":            {"--bootstrap-only", "--run-dir", "/artifacts/perf", "--dsn", "postgresql://x/y"},
		"bootstrap disabled": {"--bootstrap-only", "--bootstrap-schema=false", "--dsn", "postgresql://x/y"},
	} {
		deps := Dependencies{Bootstrap: func(context.Context, publisher.Config) error {
			t.Fatalf("%s: bootstrap must not run", name)
			return nil
		}}
