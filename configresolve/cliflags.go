...
func RegisterCLIInputsFlags(fs *flag.FlagSet) func() CLIInputs {
	...
	discoverySecret := fs.String("discovery-secret", "", "Read-only secret accepted ONLY on the discovery endpoints (GET /api/v1/warehouses, /api/v1/warehouse-team-ids), for external writers like millpond/viaduck; never grants admin access (env: DUCKGRES_DISCOVERY_SECRET)")
	discoverySecretFallbacks := fs.String("discovery-secret-fallbacks", "", "Comma-separated previous discovery secrets still accepted during rotation, newest first (env: DUCKGRES_DISCOVERY_SECRET_FALLBACKS)")
	...
	return func() CLIInputs {
		...
		cli.DiscoverySecret = *discoverySecret
		cli.DiscoverySecretFallbacks = *discoverySecretFallbacks
