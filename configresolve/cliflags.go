...
func RegisterCLIInputsFlags(fs *flag.FlagSet) func() CLIInputs {
	...
	readOnlySecret := fs.String("read-only-secret", "", "Read-only secret accepted on discovery GET endpoints and GET /v1/hogql/compatibility/semantic-catalog; never grants mutation or admin access (env: DUCKGRES_READ_ONLY_SECRET)")
