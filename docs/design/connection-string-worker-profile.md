...
A backfill dual-write does `INSERT … SELECT * FROM read_parquet(full-day)` (memory-heavy; can OOM 16Gi) — hence
...
- **Shape-aware warm pool** (was single-shape 4/16): an 8/48 request now matches a warm worker instead of permanent backpressure (`ColocatedWarmShapes`).
