...
| --- | ---: | --- |
| `max_retained_bind_bytes` | `67108864` (64 MiB) | Aggregate Bind storage retained by open portals (wire body plus compact metadata) |
...
| --- | --- |
| `duckgres_retained_bind_bytes` | Aggregate retained Bind storage bytes across the process |
...
1. Confirm the rejection reason and inspect the retained-byte and open-portal
   ...
   `54000` did not install a portal and did not change accounting.
2. For affected clients, finish executing portals, explicitly close portals or
   their statements, or send `Sync` while idle outside an explicit
   transaction. These protocol boundaries release payloads and remove portal
...
1. Check that `duckgres_portal_budget_rejections_total` stops increasing for
   the expected reason.
2. Verify terminal execution or explicit cleanup causes
   `duckgres_portal_payload_releases_total` to increase and the retained-byte
   gauge to fall.
