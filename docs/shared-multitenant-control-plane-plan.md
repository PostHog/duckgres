...
- [x] Implement team-specific DuckLake/warehouse-db/metadata-store/S3 runtime wiring.
  The control plane now selects and mounts a per-team `runtime_config` Secret, and the provisioning-side helper path can render that worker-ready Secret plus the supporting warehouse-db / metadata-store / S3 secrets from the structured managed-warehouse contract. Managed-warehouse teams now fail closed if that runtime artifact is missing, rather than falling back to a shared worker config. The local multi-tenant dev/test flow uses that same artifact-rendering path instead of a hand-written runtime Secret.
- [x] Change worker pod spec from global to be team-specific.
- [ ] Replace same-namespace worker owner-ref GC with explicit cross-namespace worker cleanup/reconciliation for per-team namespaces.
