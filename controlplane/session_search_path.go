...
// unqualified DDL/DML never reaches the warehouse.
...
// default, or "ducklake"). defaultCatalog is the per-user configured default
// ("" or "ducklake"). duckLakeAttached reflects what the worker actually
// attached for this session. The bool is false when the requested catalog isn't
// attached (caller should fail the connection 3D000) or nothing is attached.
func resolveEffectiveCatalog(requested, defaultCatalog string, duckLakeAttached bool) (string, bool) {
	switch requested {
	...
	if defaultCatalog == physicalDuckLakeCatalog {
		if duckLakeAttached {
			return physicalDuckLakeCatalog, true
		}
		return "", false
