...
func TestSharedPoolConfigResolves(t *testing.T) {
	...
	config := configs[0]
	// The pool keeps the cell's existing identity: the catalog store's cell_id,
	// the routing group and the namespace are all unchanged, because compute
	// replacement must not rewrite warehouse assignment.
