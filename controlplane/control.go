...
type ControlPlaneConfig struct {
	...
	//                   empty, use the resolved org's database fallback. Legacy
	...
	// single-label prefix is resolved as hostname_alias or DNS-safe org name.
	// Postgres still honors an explicit startup database, but only when it
...
type ConfigStoreInterface interface {
	...
	// It accepts hostname_alias and DNS-safe org names.
	ResolveSNIPrefix(prefix string) (orgID, databaseName string)
	ResolveSNIPrefixWithAlias(prefix string) (orgID, databaseName string, aliasUsed bool)
...
const (
	...
	SNIRoutingPassthrough = "passthrough" // validate managed SNI, use resolved org db only when database is empty
...
func (cp *ControlPlane) handleConnection(conn net.Conn) {
	...
	if cp.configStore != nil {
		...
		//                 to the resolved org's database. Non-managed hostnames
...
// Behavior is gated on cp.cfg.SNIRoutingMode. Flight has no startup database,
// so a managed SNI hostname directly selects the org.
...
// matched-passthrough. Translates the prefix through hostname_alias and
// DNS-safe org-name lookups so callers reach the same org regardless of which
// supported hostname label they use.
//
// Alias precedence: if a prefix matches both an org's hostname_alias AND
// another org's name, the alias wins (ResolveSNIPrefix checks the alias map
// first). Operators must avoid that collision — the admin API enforces unique
// aliases separately, but does not cross-validate that an alias isn't another
// org's name.
func (v *cpFlightCredentialValidator) authForSNIPrefix(sni, sniPrefix, username, password string) bool {
	cp := v.cp
	orgID, dbname, aliasUsed := cp.configStore.ResolveSNIPrefixWithAlias(sniPrefix)
	if orgID == "" {
		slog.Warn("Flight client SNI references unknown org.",
			"sni", sni, "sni_prefix", sniPrefix, "sni_database", dbname, "user", username)
		return false
	}
	observeSNIRoutingResolution("flight", aliasUsed)
...
func (cp *ControlPlane) startFlightIngress() {
	...
	case cp.configStore != nil && cp.orgRouter != nil:
		...
		// Flight has no startup database, so a managed hostname selects the
		// org directly. Otherwise we fall back to scanning orgs by
		// (username, password) and log a warning so legacy callers can be
		// migrated.
		orgProvider := &orgRoutedSessionProvider{
