...
// testCatalogPartition is the catalog store's partition these suites publish
// under. It is deliberately NOT derived from any pool id: the two identities are
// configured separately, and a test that spelled them the same could not see a
// writer that confused one for the other.
const testCatalogPartition = "example-pool-store"

// scopedCatalogWriter builds the production writer against a real PostgreSQL,
// as the scoped publisher role infra provisions.
func scopedCatalogWriter(t *testing.T, poolID string, store trinoPoolRevisionStore, lease configstore.TrinoPoolLease) *trinoPoolCatalogWriter {
	t.Helper()
	...
	// The partition is required and has no default, so every suite states it.
