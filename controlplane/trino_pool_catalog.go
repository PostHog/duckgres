...
const (
	...
	// envTrinoPoolCatalogCellID names the catalog store's PARTITION - the value
	// every coordinator of the cell reads with `catalog-store.cell-id`.
	//
	// It is REQUIRED with the catalog writer and has no default. The pool id
	// carries duckgres's reserved `registered:` prefix and doubles as the
	// warehouse-ASSIGNMENT key, so deriving one from the other would force a
	// cluster to spell its store partition `registered:<cell>` and would re-key
	// every coordinator's read the day a pool's logical id changes. Defaulting
	// to it would be worse than refusing: the publisher would write rows under
	// a partition no coordinator reads, and a store that answers every query
	// with an empty catalog set looks exactly like a store nobody has published
	// to yet. The chart renders this from the same value it renders the
	// coordinators' `catalog-store.cell-id` from.
	envTrinoPoolCatalogCellID = "DUCKGRES_TRINO_POOL_CATALOG_CELL_ID"
...
// trinoPoolCatalogCellIDPattern is the shape the publisher accepts for a store
// partition. It matches what the publisher itself validates, so an unusable
// value is refused when the writer is built rather than at the first
// publication - by then the rows are already somewhere nobody reads.
var trinoPoolCatalogCellIDPattern = regexp.MustCompile(`^[A-Za-z0-9_.:-]{1,255}$`)
...
type trinoPoolCatalogWriter struct {
	db *sql.DB
	// cellID is the catalog store's PARTITION: the `cell_id` column every
	// coordinator of this cell filters on. It is a Trino-side identity.
	cellID string
	// poolID is duckgres's own pool identity, which the pool row is keyed by and
	// which the held lease names. The two are deliberately separate values; the
	// checkpoint below must address this one, never the partition.
	poolID string
...
func (w *trinoPoolCatalogWriter) checkpoint(ctx context.Context, revision int64) error {
	...
	if err := w.store.RecordTrinoPoolPublicationRevision(ctx, lease, w.poolID, revision); err != nil {
...
func (w *trinoPoolCatalogWriter) publish(ctx context.Context, mutation trinocatalog.Mutation) error {
	...
	if held && w.store != nil && result.Revision > 0 {
		if err := w.store.RecordTrinoPoolPublicationRevision(ctx, lease, w.poolID, result.Revision); err != nil {
			slog.Error("Trino pool publication revision could not be recorded; tenant admission stays closed until it is.",
				"pool", w.poolID, "cell", w.cellID, "revision", result.Revision, "error", err)
...
func buildTrinoPoolCatalogWriter(poolID string, store trinoPoolRevisionStore, authority func() (configstore.TrinoPoolLease, bool), nodes provisioner.TrinoCatalogClient) (*trinoPoolCatalogWriter, error) {
	...
	// The store partition, which is a Trino-side identity rather than this
	// pool's. Resolved before the database is opened so an unusable value is a
	// startup error rather than a first-publication one.
	cellID, err := trinoPoolCatalogCellID()
	if err != nil {
		return nil, err
	}
...
// trinoPoolCatalogCellID resolves the catalog store's partition.
//
// The value must be the one the cell's coordinators carry in
// `catalog-store.cell-id`: rows written under any other partition are invisible
// to every one of them. It is REQUIRED and fails closed - unset, blank and
// unusable are one answer, because each of them means nobody can say which
// partition this publisher would write to. Defaulting to the pool id would let
// a missing setting publish a full catalog set somewhere no coordinator looks,
// which is indistinguishable from a store nothing has published to yet.
func trinoPoolCatalogCellID() (string, error) {
	cellID := strings.TrimSpace(os.Getenv(envTrinoPoolCatalogCellID))
	if cellID == "" {
		return "", fmt.Errorf("%s is enabled but %s is unset: it is the partition the cell's coordinators read with catalog-store.cell-id, and there is no safe value to assume",
			envTrinoPoolCatalogWriter, envTrinoPoolCatalogCellID)
	}
	if !trinoPoolCatalogCellIDPattern.MatchString(cellID) {
		return "", fmt.Errorf("%s=%q is not a usable catalog store partition", envTrinoPoolCatalogCellID, cellID)
	}
	return cellID, nil
}
