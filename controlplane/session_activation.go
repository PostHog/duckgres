...
var (
	sessionActivationTotal = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Lazy first-statement worker acquisitions on the exploratory tier, by outcome (success|canceled|capacity|draining|error).",
	}, []string{"outcome"})

	sessionActivationDuration = promauto.NewHistogram(prometheus.HistogramOpts{
		Name: "duckgres_session_activation_duration_seconds",
		Help: "Wall time of a lazy first-statement worker acquisition (create + session init), successful or not. A cold spawn dominates this.",
...
func (cp *ControlPlane) activateConnectionSession(
	...
	defer func() {
		...
		sessionActivationTotal.WithLabelValues(string(activationOutcomeForCode(code))).Inc()
		sessionActivationDuration.Observe(time.Since(start).Seconds())
	}()
