...
type usageStore interface {
	AggregateScanUsageMonthly(from time.Time) ([]configstore.MonthlyScanUsageRow, error)
	AggregateStorageUsageMonthly(from time.Time) ([]configstore.MonthlyStorageUsageRow, error)
	AggregateScanUsageDaily(orgID string, from time.Time) ([]configstore.DailyScanUsageRow, error)
	AggregateStorageUsageDaily(orgID string, from time.Time) ([]configstore.DailyStorageUsageRow, error)
}

// usageAPIHandler reads retained scan and storage usage independently of billing acknowledgements.
...
const (
	...
	usageMaxMonths     = 36
	// Bound daily responses for predictable chart size.
	usageDefaultDays = 14
	usageMaxDays     = 31
)
...
// (unquoted JSON number, full precision), matching the billing batch API.
...
func (h *usageAPIHandler) getMonthlyUsage(c *gin.Context) {
	...
	scan, err := h.store.AggregateScanUsageMonthly(from)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "aggregate monthly scan usage: " + err.Error()})
		return
	}
	...
	merged := make(map[key]*monthlyUsageResponseRow, len(scan)+len(storage))
	for _, r := range scan {
	...
	for _, r := range storage {
		...
		row, ok := merged[k]
		if !ok {
			row = &monthlyUsageResponseRow{Month: r.Month, OrgID: r.OrgID, TeamID: r.TeamID, BytesScanned: "0"}
			merged[k] = row
		}
...
// the merged scan + storage families for that day.
...
func (h *usageAPIHandler) getDailyUsage(c *gin.Context) {
	...
	scan, err := h.store.AggregateScanUsageDaily(orgID, from)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "aggregate daily scan usage: " + err.Error()})
		return
	}
	...
	merged := make(map[key]*dailyUsageResponseRow, len(scan)+len(storage))
	for _, r := range scan {
	...
	for _, r := range storage {
		...
		row, ok := merged[k]
		if !ok {
			row = &dailyUsageResponseRow{Date: r.Date, TeamID: r.TeamID, BytesScanned: "0"}
			merged[k] = row
		}
		row.GiBSeconds = r.GiBSeconds
		if row.SchemaName == nil {
