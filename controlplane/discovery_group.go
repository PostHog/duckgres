...
// registerDiscoveryGroup mounts the read-only discovery endpoints on their
// OWN gin group: token-only auth (no SSO, no roles) accepting the scoped
// discovery secret OR the admin internal secret. The discovery secret
...
// routes go through this function so TestDiscoveryGroupTopology can pin
// the exact surface the discovery credential reaches.
func registerDiscoveryGroup(engine *gin.Engine, discoveryTokens, adminTokens admin.TokenSet, store provisioning.Store) {
	discoveryAPI := engine.Group("/api/v1",
		admin.AnyTokenAuthMiddleware(discoveryTokens, adminTokens),
	)
	provisioning.RegisterDiscoveryAPI(discoveryAPI, store)
}

// validateDistinctDiscoverySecret refuses a discovery secret (or fallback)
...
func validateDistinctDiscoverySecret(discovery string, discoveryFallbacks []string, internal string, internalFallbacks []string) error {
	...
	check := func(v, which string) error {
		...
		if _, clash := adminValues[v]; clash {
			return fmt.Errorf("%s equals the internal secret (or one of its fallbacks): the discovery credential would silently grant admin access — use a distinct value", which)
		}
		return nil
	}
	if err := check(discovery, "discovery secret"); err != nil {
		return err
	}
	for _, v := range discoveryFallbacks {
		if err := check(v, "a discovery secret fallback"); err != nil {
			return err
		}
	}
	return nil
