...
// Each org_<i> group owns exactly one catalog named org_<i>.
func largeFixture(orgs int) GroupCatalogs {
	gc := make(GroupCatalogs, orgs+1)
	for i := 0; i < orgs; i++ {
		group := fmt.Sprintf("org_%d", i)
		gc[group] = map[string]bool{fmt.Sprintf("org_%d", i): true}
	}
	...
	for i := 0; i < orgs; i++ {
		admin[fmt.Sprintf("org_%d", i)] = true
	}
	gc[AdminGroup] = admin
	return gc
}
...
func BenchmarkAccessCatalog1000Orgs(b *testing.B) {
	...
	input := buildInput("777", "AccessCatalog", catalogResource("org_777"))
	ctx := context.Background()
...
func TestNegativeDecisionLatency(t *testing.T) {
	...
	input := buildInput("777", "AccessCatalog", catalogResource("org_42"))
	ctx := context.Background()
