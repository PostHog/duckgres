...
CREATE TABLE duckgres_trino_pools (
    ...
    -- Order desired configuration independently of the controller's authority epoch.
...
CREATE TABLE duckgres_trino_pool_instances (
    ...
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- Record both ConfigMap identities for UID-preconditioned deletion.
    worker_config_map_name      TEXT        NOT NULL DEFAULT '',
    worker_config_map_uid       TEXT        NOT NULL DEFAULT '',
    -- Name the failed member so the Gateway charges the repair budget.
    repair_for                 TEXT        NOT NULL DEFAULT '',
    failure_reason             TEXT        NOT NULL DEFAULT '',
    -- Preserve the exact coordinator and container identities admitted by the Gateway.
    coordinator_id             TEXT        NOT NULL DEFAULT '',
    coordinator_container_id   TEXT        NOT NULL DEFAULT ''
);
...
CREATE TABLE duckgres_trino_pool_publications (
    ...
    created_at               TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at               TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- Barrier revisions identify principal bindings and acknowledged configuration.
    principal_revision       TEXT        NOT NULL DEFAULT '',
    target_revision          TEXT        NOT NULL DEFAULT '',
    admitted_target_revision TEXT        NOT NULL DEFAULT '',
    -- Persist occurrence identities and retry schedules across controller restarts.
    attempt                  BIGINT      NOT NULL DEFAULT 0 CHECK (attempt >= 0),
    attempts                 BIGINT      NOT NULL DEFAULT 0 CHECK (attempts >= 0),
    next_attempt_at          TIMESTAMPTZ NULL,
    -- Replay unknown outcomes with the original intent and identical payload.
    -- Payloads contain principal identifiers and revisions, never credentials.
    pending_intent           TEXT        NOT NULL DEFAULT '' CHECK (pending_intent IN ('', 'principals', 'revoke')),
    pending_payload          JSONB       NOT NULL DEFAULT '{}' CHECK (jsonb_typeof(pending_payload) = 'object'),
