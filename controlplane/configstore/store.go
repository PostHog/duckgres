...
// FindIncompatibleHotIdleWorkerForReplacement returns one current-org hot-idle
// worker that cannot satisfy the requested image/profile and whose retirement
// can make room for a replacement spawn under the global cap. It does not
// mutate the row; callers must retire the returned snapshot through the lifecycle
// CAS path and retry the acquire decision.
func (cs *ConfigStore) FindIncompatibleHotIdleWorkerForReplacement(orgID, image, profileCPU, profileMemory string, maxOrgWorkers, maxGlobalWorkers int) (*WorkerRecord, error) {
	if orgID == "" || maxGlobalWorkers <= 0 {
		return nil, nil
	}
	...
	err := cs.db.Transaction(func(tx *gorm.DB) error {
		...
		globalActive, err := cs.countActiveWorkers(tx)
		if err != nil {
			return err
		}
		globalNonHotIdle, err := cs.countActiveWorkers(tx, "state <> ?", WorkerStateHotIdle)
		if err != nil {
			return err
		}
		if globalNonHotIdle >= int64(maxGlobalWorkers) || globalActive < int64(maxGlobalWorkers) {
			return nil
		}
		if maxOrgWorkers > 0 {
			orgActive, err := cs.countOrgAdmittingWorkers(tx, orgID, image, profileCPU, profileMemory)
			if err != nil {
				return err
			}
			if orgActive >= int64(maxOrgWorkers) {
				return nil
			}
		}
		...
		err = tx.Table(cs.runtimeTable(current.TableName())).
			Clauses(clause.Locking{Strength: "UPDATE", Options: "SKIP LOCKED"}).
			Where("state = ? AND org_id = ?", WorkerStateHotIdle, orgID).
			Where("(COALESCE(image, '') <> ? OR COALESCE(profile_cpu, '') <> ? OR COALESCE(profile_memory, '') <> ?)", image, profileCPU, profileMemory).
			Order("worker_id ASC").
			Take(&current).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
	...
	if err != nil {
		return nil, fmt.Errorf("find incompatible hot-idle worker for replacement: %w", err)
	}
