...
func (cs *ConfigStore) TryAcquireOrgConnectionLease(requestID string, maxConnections int, now time.Time) (*OrgConnectionLease, error) {
	...
	err := cs.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Exec("SELECT pg_advisory_xact_lock(?)", advisoryLockKey("duckgres:org-connections:"+request.OrgID)).Error; err != nil {
			return err
		}

		if err := cs.cleanupOrgConnectionRowsLocked(tx, request.OrgID, now); err != nil {
			return err
		}

		if request.CanceledAt != nil || request.GrantedAt != nil || !request.ExpiresAt.After(now) {
			return nil
		}

		var existing OrgConnectionLease
		if err := tx.Table(cs.runtimeTable(existing.TableName())).
			Where("request_id = ?", requestID).
		...
		if err := tx.Table(cs.runtimeTable(head.TableName())).
		...
		if maxConnections > 0 {
			count, err := cs.countActiveOrgConnectionLeases(tx, request.OrgID)
			if err != nil {
				return err
			}
