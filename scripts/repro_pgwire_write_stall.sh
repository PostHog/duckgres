...
# before the kernel collapses the socket, and the only lifecycle line emitted
# is Info-level "Client query finished.". With the fix the CP fails the write in ~60s
...
#   - Pre-fix: ~15 min of silence, then a single "Client query finished." Info line.
...
echo "      | grep -E 'Client query (started|finished)|Query execution errored'"
echo
echo "Expected timeline:"
echo "  pre-fix : ~15 min until ETIMEDOUT, then Info-level 'Client query finished.'"
