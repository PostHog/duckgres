...
| `--sni-routing-mode` | `DUCKGRES_SNI_ROUTING_MODE` | Managed-hostname routing: `off`, `passthrough`, or `enforce`. Postgres uses requested dbname first; managed SNI must resolve to the same org, and the resolved org's database is used only when dbname is empty |
...
For managed-hostname routing, `passthrough` logs legacy/non-managed callers while allowing them to route by requested dbname. `enforce` rejects Postgres clients whose TLS SNI does not match a configured managed suffix. In both managed modes, when SNI does match a suffix, the hostname prefix resolves by `hostname_alias` or DNS-safe org name and must match the requested Postgres database's org; if the client omits the startup database, the resolved org's database is used as the fallback. Unknown routing-mode values behave like `off`.
